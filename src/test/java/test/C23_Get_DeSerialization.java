...
public class C23_Get_DeSerialization extends DummyBaseURL {
    ...
    public void get01(){
        ...
        HashMap<String,Object> expBodyMap=testDataDummy.expDummyOlusturMap();
        ...
        assertEquals(expBodyMap.get("status"),respMap.get("status"));
        assertEquals(expBodyMap.get("message"),respMap.get("message"));

       assertEquals (     ((Map)(expBodyMap.get("data"))).get("id")    ,
                ((Map)(respMap.get("data"))).get("id")         );

        assertEquals(      ((Map)(expBodyMap.get("data"))).get("employee_name")    ,
                ((Map)(respMap.get("data"))).get("employee_name")       );

        assertEquals(      ((Map)(expBodyMap.get("data"))).get("employee_salary")    ,
                ((Map)(respMap.get("data"))).get("employee_salary")      );

        assertEquals(       ((Map)(expBodyMap.get("data"))).get("employee_age")     ,
                ((Map)(respMap.get("data"))).get("employee_age")       );

        assertEquals(       ((Map)(expBodyMap.get("data"))).get("profile_image")     ,
